...
# write file with a request that times out more than 3 times
def test_TimeOutOnFileWrite(client):
    with pytest.raises((socket.timeout, ConnectionResetError)):
      client.putFileFailData(b'f13b.ul', 13)


# Get file 'f3blks.bin' with wrong acknowledgement four times
def test_getFileWithWrongAckFourTimes(client):
